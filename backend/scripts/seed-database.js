...
async function creerMediasPourOeuvre(models, oeuvre) {
  ...
  for (let i = 0; i < nbImages; i++) {
    const media = await models.Media.create({
      ...
      type_media: 'image',
      url: `uploads/oeuvres/images/${oeuvre.id_oeuvre}_${i + 1}.jpg`,
      ...
      visible_public: true,
      ordre: i,
      thumbnail_url: `uploads/oeuvres/images/thumbs/${oeuvre.id_oeuvre}_${i + 1}_thumb.jpg`,
  ...
  for (let i = 0; i < nbDocuments; i++) {
    const docType = faker.helpers.arrayElement(['pdf', 'docx']);
    const docTitle = faker.helpers.arrayElement(['Fiche technique', 'Documentation', 'Analyse critique', 'Dossier de presse']);
    
    const media = await models.Media.create({
      id_oeuvre: oeuvre.id_oeuvre,
      type_media: 'document',
      url: `uploads/oeuvres/documents/${oeuvre.id_oeuvre}_${docTitle.toLowerCase().replace(/ /g, '_')}.${docType}`,
      titre: `${docTitle} - ${oeuvre.titre}`,
      description: `Document ${docTitle.toLowerCase()} de l'œuvre`,
      visible_public: true,
      ordre: nbImages + i,
      mime_type: docType === 'pdf' ? 'application/pdf' : 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
      taille_fichier: faker.number.int({ min: 100000, max: 5000000 }),
