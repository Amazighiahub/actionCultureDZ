/**
 * CategorySelection - Composant de sélection des catégories pour une œuvre
 * Extrait de AjouterOeuvre.tsx pour une meilleure modularité
 */
...
const CategorySelection: React.FC<CategorySelectionProps> = React.memo(({
    ...
    return (
      <Card className="shadow-cultural">
        <CardContent className="flex items-center justify-center py-8">
          ...
          <span className="ml-2">{t("ajouteroeuvre.chargement_des_catgories", "Chargement des catégories...")}</span>
        </CardContent>
      </Card>
    );
  }
  ...
  if (categoriesGrouped.length === 0) {
    return (
      <Card className="shadow-cultural">
        <CardHeader>
          <CardTitle className="text-2xl font-serif">{t("ajouteroeuvre.catgories_1", "Catégories")}</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-muted-foreground">
            {t("ajouteroeuvre.aucune_catgorie_disponible", "Aucune catégorie disponible")}
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card className="shadow-cultural">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground mt-2">
          {t("ajouteroeuvre.slectionnez_une_plusieurs", "Sélectionnez une ou plusieurs catégories")}
        </p>
      </CardHeader>
              ...
              <div className="flex items-center justify-between">
                ...
                {genre.categories.length > 1 && (
                  <Button
                    ...
                    className="text-xs"
                  >
                    {allSelected ? t('common.deselectAll', 'Tout désélectionner') : t('common.selectAll', 'Tout sélectionner')}
                  </Button>
                )}
              </div>
